...
class GUI
{
    ...
    public:
    GUI (int width, int height, displayFunction dFunction = glutGUI::defaultDisplay,
         keyFunction kFunction          = glutGUI::defaultKey,
         mouseButtonFunction mbFunction = glutGUI::defaultMouseButton, const char *title = "GLUT");
    void GLUTInit ();
    void GLInit ();
    ~GUI ();
    ...
    static void setLight (int id, float posx, float posy, float posz, bool onOffKeyDefault = false,
                          bool attenuated = true, bool low = false, bool hidden = false,
                          bool pontual = true, bool spot = false, bool onOffUserControl = true);
    ...
    static void pickingInit (GLint cursorX, GLint cursorY, int w, int h, GLuint *selectBuf,
                             int BUFSIZE);
    ...
    static void drawQuad (float width = 5.0, float height = 5.0, float discrWidth = 0.3,
                          float discrHeight = 0.3, float texWidth = 5.0, float texHeight = 5.0,
                          bool inverted = false);
    static void drawOriQuad (float angle = 0.0, float ex = 1.0, float ey = 0.0, float ez = 0.0,
                             float width = 5.0, float height = 5.0, float discrWidth = 0.3,
                             float discrHeight = 0.3, float texWidth = 5.0, float texHeight = 5.0,
                             bool inverted = false);
    static void drawQuadBox (float xmin, float ymin, float zmin, float xmax, float ymax,
                             float zmax, float discrWidth = 0.3, float discrHeight = 0.3,
                             bool inverted = false);
    static void drawBox (float xmin, float ymin, float zmin, float xmax, float ymax, float zmax,
                         bool inverted = false);
    static void drawScaledBox (float scale, float xmin, float ymin, float zmin, float xmax,
                               float ymax, float zmax, bool inverted = false);
    static void drawFloor (float width = 5.0, float height = 5.0, float discrWidth = 0.3,
                           float discrHeight = 0.3, float texWidth = 5.0, float texHeight = 5.0);
    static void drawPlane (GLfloat planeABCD[], float width = 5.0, float height = 5.0,
                           float discrWidth = 0.03, float discrHeight = 0.03, float texWidth = 5.0,
                           float texHeight = 5.0);
    static void drawPlaneAL (GLfloat planeABCD[], float width = 15.0, float height = 15.0,
                             float discrWidth = 0.5, float discrHeight = 0.5,
                             float texWidth = 15.0, float texHeight = 15.0);
    static void drawPlane (Vetor3D n, GLfloat distMinPlanoOrigem, float width = 5.0,
                           float height = 5.0, float discrWidth = 0.03, float discrHeight = 0.03,
                           float texWidth = 5.0, float texHeight = 5.0);
    ...
    static void draw3ds (Model3DS &model3DS, float tx = 0, float ty = 0, float tz = 0,
                         float ax = 0, float ay = 0, float az = 0, float sx = 1, float sy = 1,
                         float sz = 1);
};
