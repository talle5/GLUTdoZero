...
void glutGUI::defaultDisplay ()
{
    ...
    glLoadIdentity ();
    gluLookAt (cam->e.x, cam->e.y, cam->e.z, cam->c.x, cam->c.y, cam->c.z, cam->u.x, cam->u.y,
               cam->u.z);
...
void glutGUI::changeCam ()
{
    ...
    case 0:
        ...
        cam                   = new CameraDistante (); // CameraDistante(0,1,10, 0,1,0,
                                                       // 0,1,0);
        break;
    case 1:
    ...
    case 15:
        glutGUI::perspective  = true;
        glutGUI::pontosDeFuga = false;
        cam = new CameraDistante (savedCamera[0], savedCamera[1], savedCamera[2], savedCamera[3],
                                  savedCamera[4], savedCamera[5], savedCamera[6], savedCamera[7],
                                  savedCamera[8]);
        break;
    }
...
void glutGUI::showLocalAndGlobalCoords (float pl[4])
{
    ...
    composite ();
    // obtendo e mostrando a matriz de composicao (matriz modelView atual) no
    // console
    float compositeMatrix[16];
    glGetFloatv (GL_MODELVIEW_MATRIX, compositeMatrix);
    showGLMatrixIn2D (compositeMatrix);
    // ponto em coords globais é obtido pelo ponto em coords locais
    // transformado pela matriz de composicao
    float pg[4];
