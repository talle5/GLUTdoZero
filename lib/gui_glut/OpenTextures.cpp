...
void OpenTextures::loadTexture (const char *filename, int wrap)
{
    ...
    ilDeleteImages (1, &image); /* Because we have already copied image data into texture
                                   data we can release memory used by image. */

    // Set Texture mapping parameters
    // select modulate to mix texture with color for shading
    glTexEnvf (GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE,
               GL_MODULATE); // GL_REPLACE, GL_MODULATE, GL_BLEND
    ...
    glTexParameterf (GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                     wrap ? GL_REPEAT : GL_CLAMP); // GL_CLAMP : GL_REPEAT
...
void OpenTextures::loadTextureBMP (const char *szFileName, int wrap)
{
    /*
      //carrega a imagem
            ...
            ReadFile(hFileHandle,&bitmapHeader,sizeof(BITMAPFILEHEADER),
                    ...
                    lInfoSize = bitmapHeader.bfOffBits -
    sizeof(BITMAPFILEHEADER); pBitmapInfo = (BITMAPINFO *) new BYTE[lInfoSize];
    ...
    pBitmapInfo->bmiHeader.biBitCount + 7) / 8 * abs(nTextureHeight);
    ...
    GL_LINEAR);
        ...
        // if wrap is true, the texture wraps over at the edges (repeat)
        //       ... false, the texture ends at the edges (clamp)
        glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap ? GL_REPEAT :
    GL_CLAMP ); glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrap ?
    GL_REPEAT : GL_CLAMP );
    //*/
}
//---------------------------------------------------------------------------
void OpenTextures::read_JPEG_file (char *filename, int wrap)
{
    /*
        ...
        jpeg_create_decompress(&cinfo);
            ...
            big_buff = (unsigned char *) malloc(cinfo.output_height *
    cinfo.output_width * cinfo.output_components);

            while (cinfo.output_scanline < cinfo.output_height)
            {
                    JDIMENSION read_now = jpeg_read_scanlines(&cinfo, buffer,
    1); memcpy(&big_buff[(cinfo.output_scanline - read_now) *
    cinfo.output_width * cinfo.output_components], buffer[0], row_stride);
            }

        //*
        //* We call gluBuild2DMipmaps to load the texture in opengl's memory
    and create mipmaps
        //*
            gluBuild2DMipmaps(GL_TEXTURE_2D, 3, cinfo.output_width,
    cinfo.output_height, GL_RGB, GL_UNSIGNED_BYTE, big_buff);
        //*
